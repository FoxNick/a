...
public abstract class PageLoader {
    ...
    int mFirstPageMarginTop = 300;//標題上面空白

    int mFirstPageMarginButtom = 200;//標題下方空白
    ...
    private void initData() {
        ...
        progressDisplay = readBookControl.getProgressDisplay();
    ...
    private void initPaint() {
        ...
        float boldSize = readBookControl.getBoldSize()>0 ? readBookControl.getBoldSize():0;
        mTextPaint.setStyle(Paint.Style.FILL_AND_STROKE);
        mTextPaint.setStrokeWidth(boldSize);
    ...
    public int getBrighterColor(int color){
        ...
        hsv[1] = hsv[1] - 0.4f; // less saturation
        hsv[2] = hsv[2] + 0.4f; // more brightness
    ...
    private synchronized void drawContent(Bitmap bitmap, TxtChapter txtChapter, TxtPage txtPage) {
        ...
        if (txtChapter.getStatus() != TxtChapter.Status.FINISH) {
            ...
            for (int i = 0; i < txtPage.getTitleLines(); ++i) {
                ...
                for (TxtChar c : txtPage.txtLists.get(i).CharsData) {
            ...
            for (int i = txtPage.getTitleLines(); i < txtPage.size(); ++i) {
                ...
                if (needScale(str)) {
                    ...
                    drawScaledText(canvas, str, width, mTextPaint, top,i,txtPage.txtLists);
        ...
        while (top < totalHeight) {
            ...
            for (int i = 0; i < page.getTitleLines(); i++) {
                if (top > totalHeight) {
                    ...
                    for (TxtChar c : page.txtLists.get(i).CharsData) {
                        rightposition = leftposition + c.charWidth;
                        Point tlp = new Point();
                        c.TopLeftPosition = tlp;
                        tlp.x = (int) leftposition;
            ...
            for (int i = page.getTitleLines(); i < page.size(); i++) {
                ...
                if (top > totalHeight) {
                    ...
                    if (needScale(str)) {
                        //drawScaledText(canvas, str, width, mTextPaint, top);
                        drawScaledText(canvas, str, width, mTextPaint, top,i,page.txtLists);
    ...
    TxtChar DetectPressTxtChar(float down_X2, float down_Y2) {
        ...
        // int i = 0;
        for (TxtLine l : curChapter().txtChapter.getPage(mCurPagePos).txtLists) {
            //   if(i< mCurChapter.getPage(mCurPagePos).titleLines) {
            //      i++;
            //     continue;
            //  }
