package com.kunfei.bookshelf.widget.settingview;
...
public class LSettingItem extends RelativeLayout {
    ...
    public void getCustomStyle(Context context, AttributeSet attrs) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            else if (attr == R.styleable.LSettingView_introTextColor) {
                //文字默认灰色
                mIntroTextColor = a.getColor(attr, Color.GRAY);
                mTvIntroText.setTextColor(mRightTextColor);
            }else if (attr == R.styleable.LSettingView_introText) {
                mTvIntroText.setText(a.getString(attr));
            }
        }
        a.recycle();
    }
     ...
     * 更改checkbox选择状态
     */
    public void setRightIconCheck(Boolean isChecked) {
